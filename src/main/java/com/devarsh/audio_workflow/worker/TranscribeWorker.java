...
public class TranscribeWorker {
    ...
    @RabbitListener(queues = "q.transcribe")
    public void handle(TaskMessage message) {
        log.info("TranscribeWorker received task {}", message.taskId());
        workflowStateService.markTaskInProgress(message.taskId(), "worker.transcribe");

        try {
            Thread.sleep(500);
            rabbitTemplate.convertAndSend("workflow.exchange", "results",
                    new TaskResultMessage(message.taskId(), message.workflowId(),
                            message.taskType(), true, null, Map.of("transcript", "sample transcript text")));
            log.info("TranscribeWorker completed task {}", message.taskId());
        } catch (Exception e) {
            log.error("TranscribeWorker failed task {}", message.taskId(), e);
            rabbitTemplate.convertAndSend("workflow.exchange", "results",
                    new TaskResultMessage(message.taskId(), message.workflowId(),
